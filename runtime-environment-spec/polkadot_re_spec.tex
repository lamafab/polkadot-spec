...
\usepackage{geometry,amsmath,amssymb,hyperref,calc,ifthen,verbatim,theorem,capt-of,makeidx}
\makeindex
...
\newenvironment{tmcode}[1][]{\begin{verbatim} }{\end{verbatim}}
...
\custombinding{6}{\noindent}\begin{tmparmod}{0pt}{0pt}{0em}%
    ...
    \begin{tmindent}
      {\tmname{Import-and-Validate-Block($B, \tmop{Just} (B)$)}}
    \end{tmindent}
\end{tmparmod}{\medskip}
...
{\todo{Describe how the results are going to impact the blocks etc}}

\

As Offchain Workers run on their own execution environment they have access
to their separate storage. There are two different types of storage available
as defined in Definitions \ref{defn-offchain-persistent-storage} and
...
\begin{itemize}
  ...
  encoded network state. {\todo{Spec the detail of the the result this
...
\subsection{\tmverbatim{BabeApi\_epoch}}\label{sect-rte-babeapi-epoch}
